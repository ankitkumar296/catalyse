...
class vgg16_in():
    def __init__(self,DNN_profile,DNN_weights,trans_data):
    ...
    def load_layer_old(self, layer_id):
        ...
        self.layer.set_weights(self.weight_list[layer_id])
    ...
    def execute_threshold_switching_layer_by_layer(self,input_data,cpu_log,threshold):
        self.executed_map=[]
        # print('Helo')
        ...
        lock=True
        for lay in range(0,self.NO_OF_LAYERS):
            # print('In layer loop')
            ...
            if(lock):
                ...
                if(self.out_trans[lay] <= threshhold):
            ...
            # Loading cluster layers
            # curr_cluster=[]
            # gc.collect()
            t41=time.perf_counter()
            # for j in range(cluster_size):
            #     # print(f"------{lay+j}----")
            #     curr_cluster.append(self.load_layer(lay+j))
            #     if j+lay == 21: 
            #         break
