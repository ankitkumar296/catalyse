...
from dnn_models.random_cpu.mtl_net_random import *
from dnn_models.random_cpu.pilotnet_random import *
from dnn_models.random_cpu.SD_random import *
from dnn_models.random_cpu.squeeznet_random import *
from dnn_models.random_cpu.vgg16_random import *
...
def child(worker: int,cpu_log,mode:int,input_data) -> None:
    ...
    r2=obj.execute_layer_by_layer_random(input_data=input_data,cpu_log=cpu_log)
...
def main(para=1) -> None:
    ...
    main_et_file=f'./readings/random/inst_set_{NO_OF_INFERENCE}_delay/main_ET.csv'
    ...
    for inst in range(NO_OF_INFERENCE):
        ...
        if inst in [0,4,8,16,20,18]:
            #VGG inference    
            updators[inst]=update_class(mode=22,read_file_name=f'./readings/random/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=vgg_deadline)
            inputs[inst]=np.random.rand(1, 224, 224, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,22,inputs[inst]),callback=updators[inst].updater)
        elif inst in [3,6,9,17,23,30]:
            #Squeeznet inference
            updators[inst]=update_class(mode=44,read_file_name=f'./readings/random/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=squeeznet_deadline)
            inputs[inst]=np.random.rand(1, 224, 224, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,44,inputs[inst]),callback=updators[inst].updater)
        elif inst in [2,10,12,14,18,22,24,26]:
            #Pilotnet inference
            updators[inst]=update_class(mode=11,read_file_name=f'./readings/random/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=pilotnet_deadline)
            inputs[inst]=np.random.rand(1, 66, 200, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,11,inputs[inst]),callback=updators[inst].updater)
        elif inst in [1,5,7,21,29,31]:
            #Custom MTL inference
            updators[inst]=update_class(mode=30,read_file_name=f'./readings/random/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=custom_mtl_deadline)
            inputs[inst]=np.random.rand(1, 32, 32, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,30,inputs[inst]),callback=updators[inst].updater)
        elif inst in [11,13,15,19,25,27]:
            #Self Driving MTL inference
            updators[inst]=update_class(mode=18,read_file_name=f'./readings/random/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=self_driving_mtl_deadline)
            inputs[inst]=np.random.rand(1, 140, 208, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,18,inputs[inst]),callback=updators[inst].updater)
