...
from dnn_models.greedy_wc.mtl_net_greedy_wc import *
from dnn_models.greedy_wc.pilotnet_greedy_wc import *
from dnn_models.greedy_wc.SD_greedy_wc import *
from dnn_models.greedy_wc.squeeznet_greedy_wc import *
from dnn_models.greedy_wc.vgg16_greedy_WC import *

def child(worker: int,cpu_log,mode:int,input_data,DNN_profile,DNN_weights) -> None:
    ...
    if mode==11:
        obj=pilotnet(prof_data=DNN_profile,DNN_weights=DNN_weights)
    elif mode == 22:
        obj=vgg16_in(prof_data=DNN_profile,DNN_weights=DNN_weights)
    elif mode == 44:
        obj=squeeznet(DNN_profile=DNN_profile,DNN_weights=DNN_weights)
        # r1=obj.make_partition()
    elif mode == 30:
        obj=custom_MTL(DNN_profile=DNN_profile,DNN_weights=DNN_weights)
    elif mode == 18:
        obj = self_driving_mtl(DNN_profile=DNN_profile,DNN_weights=DNN_weights)
    ...
    r2=obj.execute_greedy_wc_layer_by_layer(input_data=input_data,cpu_log=cpu_log)
...
def main(para=1) -> None:
    ...
    main_et_file=f'./readings/greedy_wc/inst_set_{NO_OF_INFERENCE}_delay/main_ET.csv'
    ...
    for inst in range(NO_OF_INFERENCE):
        ...
        if inst in [0,4,8,16,20,18]:
            #VGG inference
            updators[inst]=update_class(mode=22,read_file_name=f'./readings/greedy_wc/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=vgg_deadline)
            inputs[inst]=np.random.rand(1, 224, 224, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,22,inputs[inst],vgg_profile,weight_list_vgg16),callback=updators[inst].updater)
        elif inst in [3,6,9,17,23,30]:
            #Squeeznet inference
            updators[inst]=update_class(mode=44,read_file_name=f'./readings/greedy_wc/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=squeeznet_deadline)
            inputs[inst]=np.random.rand(1, 224, 224, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,44,inputs[inst],squeeznet_profile,weight_list_squeeznet),callback=updators[inst].updater)
        elif inst in [2,10,12,14,18,22,24,26]:
            #Pilotnet inference
            updators[inst]=update_class(mode=11,read_file_name=f'./readings/greedy_wc/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=pilotnet_deadline)
            inputs[inst]=np.random.rand(1, 66, 200, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,11,inputs[inst],pilotnet_profile,weight_list_pilotnet),callback=updators[inst].updater)
        elif inst in [1,5,7,21,29,31]:
            #Custom MTL inference
            updators[inst]=update_class(mode=30,read_file_name=f'./readings/greedy_wc/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=squeeznet_deadline)
            inputs[inst]=np.random.rand(1, 32, 32, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,30,inputs[inst],custom_mtl_profile,weight_list_custom_mtl),callback=updators[inst].updater)
        elif inst in [11,13,15,19,25,27]:
            #Self Driving MTL inference
            updators[inst]=update_class(mode=18,read_file_name=f'./readings/greedy_wc/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=squeeznet_deadline)
            inputs[inst]=np.random.rand(1, 140, 208, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,18,inputs[inst],self_driving_mtl_profile,weight_list_self_driving_mtl),callback=updators[inst].updater)
