...
from dnn_models.naive_cpu.mtl_net_naive_cpu import *
from dnn_models.naive_cpu.pilotnet_naive_cpu import *
from dnn_models.naive_cpu.SD_naive_cpu import *
from dnn_models.naive_cpu.squeeznet_naive_cpu import *
from dnn_models.naive_cpu.vgg16_naive_cpu import *
...
def child(worker: int,cpu_log,mode:int,input_data,DNN_profile) -> None:
    ...
    if mode==11:
        obj=pilotnet(prof_data=DNN_profile)
    elif mode == 22:
        obj=vgg16_in(prof_data=DNN_profile)
    elif mode == 44:
        obj=squeeznet(DNN_profile=DNN_profile)
        # r1=obj.make_partition()
    elif mode == 30:
        obj = custom_MTL(DNN_profile=DNN_profile)
    elif mode == 18:
        obj = self_driving_mtl(DNN_profile=DNN_profile)
    ...
    r2=obj.execute_naive_cpu_layer_by_layer(input_data=input_data,cpu_log=cpu_log)
...
def main(para=1) -> None:
    ...
    main_et_file=f'./readings/naive_cpu/inst_set_{NO_OF_INFERENCE}_delay/main_ET.csv'
    ...
    with open('./dnn_models/_profilings/naive_profile/pilotnet_naive_cpu.pkl', 'rb') as file:
        pilotnet_profile = pickle.load(file)
    with open('./dnn_models/_profilings/naive_profile/vgg_naive_cpu.pkl', 'rb') as file:
        vgg_profile = pickle.load(file)
    with open('./dnn_models/_profilings/naive_profile/squeeznet_naive_cpu.pkl', 'rb') as file:
        squeeznet_profile = pickle.load(file)
    with open('./dnn_models/_profilings/naive_profile/custom_mtl_naive_cpu.pkl', 'rb') as file:
        custom_mtl_profile = pickle.load(file)
    with open('./dnn_models/_profilings/naive_profile/self_driving_mtl_naive_cpu.pkl', 'rb') as file:
    ...
    for inst in range(NO_OF_INFERENCE):
        ...
        if inst in [0,4,8,16,20,18]:
            #VGG inference
            updators[inst]=update_class(mode=22,read_file_name=f'./readings/naive_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=vgg_deadline)
            inputs[inst]=np.random.rand(1, 224, 224, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,22,inputs[inst],vgg_profile),callback=updators[inst].updater)
        elif inst in [3,6,9,17,23,30]:
            #Squeeznet inference
            updators[inst]=update_class(mode=44,read_file_name=f'./readings/naive_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=squeeznet_deadline)
            inputs[inst]=np.random.rand(1, 224, 224, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,44,inputs[inst],squeeznet_profile),callback=updators[inst].updater)
        elif inst in [2,10,12,14,18,22,24,26]:
            #Pilotnet inference
            updators[inst]=update_class(mode=11,read_file_name=f'./readings/naive_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=pilotnet_deadline)
            inputs[inst]=np.random.rand(1, 66, 200, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,11,inputs[inst],pilotnet_profile),callback=updators[inst].updater)
        elif inst in [1,5,7,21,29,31]:
            #Custom MTL inference
            updators[inst]=update_class(mode=30,read_file_name=f'./readings/naive_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=custom_mtl_deadline)
            ...
            updators[inst]=update_class(mode=18,read_file_name=f'./readings/naive_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=self_driving_mtl_deadline)
