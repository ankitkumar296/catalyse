...
def main(para=1) -> None:
    ...
    main_et_file=f'./readings/sota_all_cpu/inst_set_{NO_OF_INFERENCE}_delay/main_ET.csv'
    ...
    for inst in range(NO_OF_INFERENCE):
        ...
        if inst in [0,4,8,16,20,18]:
            #VGG inference
            updators[inst]=update_class(mode=22,read_file_name=f'./readings/sota_all_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=vgg_deadline)
            ...
            #Squeeznet inference
            updators[inst]=update_class(mode=44,read_file_name=f'./readings/sota_all_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=squeeznet_deadline)
            inputs[inst]=np.random.rand(1, 224, 224, 3).astype(np.float32)
            pool.apply_async(child, (0,cpu_log,44,inputs[inst]),callback=updators[inst].updater)
        elif inst in [2,10,12,14,18,22,24,26]:
            #Pilotnet inference
            updators[inst]=update_class(mode=11,read_file_name=f'./readings/sota_all_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=pilotnet_deadline)
            ...
            updators[inst]=update_class(mode=30,read_file_name=f'./readings/sota_all_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=custom_mtl_deadline)
            ...
            updators[inst]=update_class(mode=18,read_file_name=f'./readings/sota_all_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv',deadline=self_driving_mtl_deadline)
