...
class self_driving_mtl():
    def __init__(self,self_driving_mtl_profile,DNN_weights=None,trans_data=None) -> None:
    ...
    def execute_layer_by_layer_sample_inp(self,input_data,cpu_log,threshold):
...
def child(worker: int,cpu_log,mode:int,input_data,DNN_profile,DNN_weights,threshold,trans_data) -> None:
    ...
    if mode==18:
        obj=self_driving_mtl(self_driving_mtl_profile=DNN_profile,DNN_weights=DNN_weights,trans_data=trans_data)
    elif mode == 7:
        obj=self_driving_mtl(self_driving_mtl_profile=DNN_profile,DNN_weights=DNN_weights,trans_data=trans_data)
    ...
    r2=obj.execute_layer_by_layer_sample_inp(input_data=input_data,cpu_log=cpu_log,threshold=threshold)
...
def main(label,threshold) -> None:
    ...
    with open('./sd_mtl_out_data_trans.pkl','rb') as file:
    ...
    up1=update_class(mode=18,read_file_name=f'./readings/threshold_switch/threshold_{label}_inf1.csv')
    up2=update_class(mode=18,read_file_name=f'./readings/threshold_switch/threshold_{label}_inf2.csv')
    # up3=update_class(mode=18,read_file_name='./readings/sota/lbl_all/inf3.csv')
    # up4=update_class(mode=9)
    inp1 = np.random.rand(1, 140, 208, 3).astype(np.float32)
    inp2 = np.random.rand(1, 140, 208, 3).astype(np.float32)
    # inp3 = np.random.rand(1, 140, 208, 3).astype(np.float32)
    ...
    # cpu_log.remove(0)
    pool.apply_async(child, (1,cpu_log,18,inp1,self_driving_mtl_profile,main_weight_list,THRESHHOLD,trans_data),callback=up1.updater)
    # cpu , cpu_log= get_best_cpu(up2,cpu_log)
    # cpu_log.remove(1)
    pool.apply_async(child, (5,cpu_log,18,inp2,self_driving_mtl_profile,main_weight_list,THRESHHOLD,trans_data),callback=up2.updater)
    # pool.apply_async(child, (9,cpu_log,18,inp3),callback=up3.updater)
...
if __name__ == '__main__':
    ...
    threshhold=[32,128,256,1*1024,4*1024,16*1024,128*1024,256*1024,512*1024,1024*1024]
    label=['32B','128B','256B','1KB','4KB','16KB', '128KB', '256KB', '512KB','1MB']
    para=int(sys.argv[1])
    gc.collect()
