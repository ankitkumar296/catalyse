...
def main(para=1) -> None:
    ...
    for inst in range(NO_OF_INFERENCE):
        ...
        updators[inst]=update_class(mode=44,read_file_name=f'./readings/sota_single_cpu/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv')
        inputs[inst]=np.random.rand(1, 224, 224, 3).astype(np.float32)
        sing_cp=inst%NO_OF_CPU
        pool.apply_async(child, (sing_cp,cpu_log,44,inputs[inst]),callback=updators[inst].updater)
