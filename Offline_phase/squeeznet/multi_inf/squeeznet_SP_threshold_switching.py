...
class squeeznet():
    def __init__(self,DNN_profile=None,DNN_weights=None,trans_data=None) -> None:
    ...
    def execute_lbl_serial(self, input_data,cpu_log,threshold):
...
def child(worker: int,cpu_log,mode:int,input_data,DNN_profile,DNN_weights,threshold,trans_data) -> None:
    ...
    r2=obj.execute_lbl_serial(input_data=input_data,cpu_log=cpu_log,threshold=threshold)
    ...
    return r2[0], r2[1],r2[2],r2[3],r2[4],r2[5],el2,el3
...
class update_class():
    ...
    def updater(self,token):
        ...
        self.write2csv([self.layer_ex_time,self.cpu_delay,self.layer_loading_time,self.obj_time,self.total_inf_time,self.cpu_track,self.totl])
...
def main(label,threshold) -> None:
    ...
    main_et_file=f'./readings/threshold_switch/threshold_{label}_main_ET_greedy.csv'
    ...
    with open('./squeeznet_out_data_trans.pkl','rb') as file:
    ...
    up1=update_class(mode=44,read_file_name=f'./readings/threshold_switch/threshold_{label}_inf1_greedy.csv')
    up2=update_class(mode=44,read_file_name=f'./readings/threshold_switch/threshold_{label}_inf2_greedy.csv')
    ...
    # cpu_log.remove(0)
    pool.apply_async(child, (1,cpu_log,44,inp1,squeeznet_profile,main_weight_list,THRESHHOLD,trans_data),callback=up1.updater)
    # cpu , cpu_log= get_best_cpu(up2,cpu_log)
    # cpu_log.remove(1)
    pool.apply_async(child, (5,cpu_log,44,inp2,squeeznet_profile,main_weight_list,THRESHHOLD,trans_data),callback=up2.updater)
...
if __name__ == '__main__':
    ...
    thresholds=[32*1024,128*1024,256*1024,512*1024,1024*1024,2*1024*1024]
    labels=['32KB', '128KB', '256KB', '512KB','1MB','2MB']
    for label,threshold in zip(labels,thresholds):
        ...
        for j in range(25):
            gc.collect
            main(label=label,threshold=threshold)
            gc.collect()
            time.sleep(0.05)
        gc.collect()
        time.sleep(1)
