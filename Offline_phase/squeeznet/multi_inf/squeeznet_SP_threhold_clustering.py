...
class squeeznet():
    def __init__(self,DNN_profile=None,DNN_weights=None) -> None:
    ...
    def execute_lbl_serial(self, input_data,cpu_log):
        
        # print("--------------First_-----------")
        t1=time.perf_counter()
        self.executed_map=[]
        best_proc_ele=0
        static_clust=[4, 4, 10, 5, 4, 5, 5, 1, 1, 4, 1]
        self.layer_id=0
        p=psutil.Process()
        self.buffer=[None,None,None,None]
        self.buffer[0]=input_data
        ...
        for cluster in static_clust:
            # print("---------------Entering LOOP-----------------")
            t31=time.perf_counter()
            # print("------------------------CALLING CPU_____________")
            best_proc_ele,cpu_log= self.get_best_cpu(self.layer_id,cpu_log,best_proc_ele)
            t_getcpu=time.perf_counter()
            p.cpu_affinity([best_proc_ele])
            # print('-----------------CPU_SET-------------------')
            self.executed_map.append(best_proc_ele)
            curr_cluster=[]
            curr_clust_layer_ids=[]
            t41=time.perf_counter()
            # curr_lay=self.load_layer(idx)
            for j in range(cluster):
                # print(f"------LOADING   {self.layer_id}----")
                curr_cluster.append(self.load_layer(self.layer_id))
                curr_clust_layer_ids.append(self.layer_id)
                self.layer_id+=1
                if self.layer_id == 43: 
                    break
            t12=time.perf_counter()
            for curr_lay, lay_id in zip(curr_cluster,curr_clust_layer_ids):
                # print(f'############Executing  LAYER                                                                                                                                                                             {lay_id}')
                match self.map[lay_id][0]:
                    
                    case -1:
                        self.buffer[self.map[lay_id][2][0]]=input_data
                    case 0:
                        self.buffer[self.map[lay_id][2][0]]=curr_lay(self.buffer[self.map[lay_id][1][0]])
                    case 1:
                        self.buffer[self.map[lay_id][2][0]]=self.buffer[self.map[lay_id][2][1]]=curr_lay(self.buffer[self.map[lay_id][1][0]])
                    case 2:
                        self.buffer[self.map[lay_id][2][0]]=curr_lay([self.buffer[self.map[lay_id][1][0]],self.buffer[self.map[lay_id][1][1]]])
                    case 3:
                        self.buffer[self.map[lay_id][2][0]]=self.buffer[self.map[lay_id][2][1]]=curr_lay([self.buffer[self.map[lay_id][1][0]],self.buffer[self.map[lay_id][1][1]]])
        ...
        return out,elapsed_time,layer_execution_time,select_cpu_time,layer_loading_time,self.executed_map,get_cpu_time
...
def child(worker: int,cpu_log,mode:int,input_data,DNN_profile,DNN_weights) -> None:
    ...
    r2=obj.execute_lbl_serial(input_data=input_data,cpu_log=cpu_log)
    ...
    return r2[0], r2[1],r2[2],r2[3],r2[4],r2[5],el2,r2[6],el3
...
class update_class():
    ...
    def updater(self,token):
        ...
        self.write2csv([self.layer_ex_time,self.cpu_delay,self.layer_loading_time,self.obj_time,self.total_inf_time,self.get_cpu_time,self.cpu_track,self.totl])
        print("CSV file UPDATED")
        # print(sum(self.inter_result[0]))
...
def main(para=1) -> None:
    ...
    main_et_file=f'./readings/static_clustering/inst_set_{NO_OF_INFERENCE}_delay/main_ET_static_cluster_{cluster_thresh}.csv'
    ...
    for layer_id in range(NO_OF_LAYERS):
        ...
        fname=f'./../pickled_layers/squeeznet_layer_{layer_id}.pkl'
        with open(fname, 'rb') as f:
            layer_config = pickle.load(f)
    ...
    with open('./squeeznet_greedy_profile_full.pkl', 'rb') as file:
    ...
    for inst in range(NO_OF_INFERENCE):
        ...
        updators[inst]=update_class(mode=44,read_file_name=f'./readings/static_clustering/inst_set_{NO_OF_INFERENCE}_delay/inf{inst}_greedy.csv')
        inputs[inst]=np.random.rand(1, 224, 224, 3).astype(np.float32)
        pool.apply_async(child, (0,cpu_log,44,inputs[inst],squeeznet_profile,main_weight_list),callback=updators[inst].updater)
